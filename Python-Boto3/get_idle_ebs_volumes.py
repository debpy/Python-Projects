#This program will print IDLE  EBS volumes 
import boto3
...
#            print(v)
        ###Find volumes that are not attached and in available state
            if v.attachments == [] and  v.state=='available' and len(metrics):
            #if len(metrics):
...
#                print("Metrics=",metrics)
                ...
                for metric in metrics:
                    print("Minimum Metric=",metric['Minimum'])
                ...
                #print("Result=",result)
                            ...
                            vol_dict.update({"VolumeId":v.volume_id,"State":v.state,"Type":v.volume_type,"Iops":v.iops,"CreateTime":v.create_time,"Size":v.size,"AvailabilityZone":v.availability_zone,"Snapshot Id":v.snapshot_id})
